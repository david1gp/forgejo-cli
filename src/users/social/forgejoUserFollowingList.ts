...
export async function forgejoUserFollowingList(
  transport: ForgejoRestTransport,
  userInput?: unknown,
): Promise<ForgejoResult<ForgejoUser[]>> {
  const op = "forgejoUserFollowingList"
  let path = "/api/v1/user/following"
  if (userInput !== undefined) {
    const user = forgejoUserReferenceParse(userInput)
    if (!user.success) return createResultError(op, user.errorMessage)
    path = `/api/v1/users/${encodeURIComponent(user.data)}/following`
  }
